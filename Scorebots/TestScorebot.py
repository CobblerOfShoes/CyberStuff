...
class Task:
    ...
    def isFixed(self):
        ...
        #runs the linux boolean command to see the result
            ...
            return False
#~~~~~~~~~~~~~~~~~~~~~~THINGS TO SCORE~~~~~~~~~~~~~~~~~~~~~~~~~#
users = [User(mainUser), User(shoemanus), User(ahri)] #If a user is deleted, you get a penalty
services = [] #If a service is down, you get a penalty
allTasks = [
	Task('Beginner','User ahri created', 3, '[ "$(grep ahri /etc/passwd)" ]'),
	Task('Beginner','User ahri needs new password', 3, '[ "$(grep "passwd \-e" /thereisnobashhistory?)" ]'),
	Task('Beginner','User shoemanus is an admin', 3, '[ "$(groups shoemanus | grep -e adm -e sudo)" ]'),
	Task('Beginner','User Cody is destroyed', 3, '[ ! "$(grep cody /etc/passwd)" ]'),
	Task('Returner','Bad sudoers line in /etc/sudoers.d/README', 5, '[ ! "$(grep "NOPASSWD" /etc/sudoers.d/README)" ]'),
	Task('Returner','Bad sudoers line in /etc/sudoers.d/"   "', 5, '[ ! "$(grep "\!authenticate" /etc/sudoers.d/"   ")" ]'),
	Task('Returner','/etc/group is now editable"   "', 5, '[ ! "$(lsattr /etc/group | grep a)" ]'),
	]
